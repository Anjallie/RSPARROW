...
###########################################################
  ...
  huc8 <- substr(huc,1,8)  
  huc4 <- substr(huc,1,4)  
  # Geographical Filtering for Southeast United States
  # Focusing on HUCs '08', '12', and '04'
  southeast_hucs <- c('08', '12', '04')
  is_southeast <- huc2 %in% southeast_hucs
#  table(huc4)           
#  table(huc8)
#  table(huc2)
  ...
  replaceNAs(named.list(staid,iftran,demtarea,demiarea,frac,rchtype,calsites,headflag,meanq,rchtot,
                        hload,fnode,tnode,depvar,urban,wetlands,forest,pasture))     # replace NAs with zeros
  #replace O With .0001
  
  demiarea <- ifelse(demiarea==0, 0.0001, demiarea)
 ...
 names <- named.list(forest,wetlands,urban,pasture,crops)
#   replaceNAs(names)
#   
#crops <- crops / 1000000
 ...
 landUseArea <- crops+forest+wetlands+urban+pasture
  urban_percent <- urban / landUseArea *100
  crops_percent <- crops_percent/ landUseArea * 100
  wetlands_percent <- crops / landUseArea * 100
